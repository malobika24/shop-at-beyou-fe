const mainController = function($scope, databaseService, $location) {
    ...
    function loadItemsByBrand(brand) {
        databaseService.getFromDatabase(`/api/getitems?brand=${brand}&&page=${$scope.currentPage}&&limit=${$scope.itemsPerPage}`)
            .then((items) => {
                $scope.items = items;
            });
    }
    ...
    $scope.brands = function() {
        const arr = ["Reflex", "MLANM", "Gildan", "IWOLLENCE", "PAUL JONES", "MUSHARE"];
        return arr;
    };
